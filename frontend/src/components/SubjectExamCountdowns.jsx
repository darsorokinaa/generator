...
const COMPARISON_FACTORIES = [
  (sec) =>
    `Моргания ~17/мин — это как ${fmtInt(sec * (17 / 60))} раз моргнуть (обычно мы моргаем 17 раз в минуту).`,
  (sec) =>
    `Короткие ролики ~34 с — это как посмотреть ${fmtInt(Math.floor(sec / 34))} тиктоков или рилс подряд.`,
  (sec) =>
    `Друзья — часы общения подряд — это как ${(sec / 3600).toLocaleString("ru-RU", { maximumFractionDigits: 1 })} часов непрерывно болтать с друзьями.`,
  (sec) =>
    `Песни ~3,5 мин (210 с) — это как прослушать ${fmtInt(Math.floor(sec / 210))} песен Вани Дмитриенко подряд.`,
  (sec) =>
    `Макароны ~10 мин — это как непрерывно варить макароны ${fmtInt(Math.floor(sec / 600))} раз по 10 мин (типичное время до al dente).`,
  (sec) =>
    `Ходьба ~5 км/ч — это как пройти ${fmtInt((sec / 3600) * 5)} км пешком со скоростью ~5 км/ч.`,
  (sec) =>
    `Чтение ~2,5 мин/стр. (150 с) — это как прочитать примерно ${fmtInt(Math.floor(sec / 150))} страниц художки.`,
  (sec) =>
    `Чайник ~7 мин — как вскипятить чайник ${fmtInt(Math.floor(sec / 420))} раз подряд.`,
  (sec) =>
    `Чистка зубов 2 мин — это как ${fmtInt(Math.floor(sec / 120))} двухминутных чисток зубов подряд (типичная рекомендация стоматологов).`,
  (sec) =>
    `Планка по 45 с — это как сделать ${fmtInt(Math.floor(sec / 45))} подходов планки по 45 секунд.`,
  (sec) =>
    `«Ну как ты?» ~5 с — это как написать «ну как ты?» ${fmtInt(Math.floor(sec / 5))} раз — столько раз тебя отвлекали от учёбы друзья.`,
  (sec) =>
    `Игра в Симс ~2 ч/сессия — это как сыграть ${fmtInt(Math.floor(sec / (2 * 3600)))} сессий в Симс.`,
  (sec) =>
    `Рик и Морти ~22 мин — это как посмотреть ${fmtInt(Math.floor(sec / (22 * 60)))} серий Рика и Морти.`,
];
